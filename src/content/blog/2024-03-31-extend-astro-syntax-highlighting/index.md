...
Little overdue, but there are good news! As of Astro v4.6, this whole workaround is no longer needed!

Starting with v4.6, `shikiConfig.langs` went back to _extending_ the built-in
language list instead of replacing it. Astro now lazy-loads the languages that Shiki
ships with on demand ([PR #10618](https://github.com/withastro/astro/pull/10618)), so
`langs` is only needed to add your custom grammars:
...
import caddyLang from 'path/to/caddyfile.tmLanguage.json' with { type: 'json' }
...
export default defineConfig({
    ...
    markdown: {
        shikiConfig: {
            // ...
            langs: [
                // 3. Then we add our custom language(s) to the list
                {
                    id: 'caddyfile',
...
That's it! No more manually opting in to every language, they all keep working out of the box.

Funny enough, the _replacing_ behavior only lived from Astro v4.0 to v4.5 — I published
this post just 11 days before Astro v4.6 made it obsolete. If you keep the old snippet
around, it still works, but you'll be eagerly loading every bundled grammar and losing
the lazy-loading benefit.
...
[Markdown (or MDX)](https://docs.astro.build/en/guides/syntax-highlighting/#markdown-code-blocks) or their
[`<Code>`](https://docs.astro.build/en/guides/syntax-highlighting/#code-) component. Such integration saves a lot
of time and makes it
[pretty easy to customize](https://docs.astro.build/en/reference/configuration-reference/#markdownshikiconfig).
...
[`markdown.shikiConfig.langs`](https://docs.astro.build/en/reference/configuration-reference/#markdownshikiconfig)
