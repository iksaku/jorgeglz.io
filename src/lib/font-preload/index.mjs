...
export default {
  ...
  hooks: {
    'astro:build:done': async ({ dir }) => {
      ...
      const stylesheets = (await readdir(assets)).filter((filename) => filename.endsWith('.css'))
      ...
      for (const page of await readdir(dir, { recursive: true })) {
        ...
        const fontsToPreload = new Set(stylesInPage.flatMap((stylesheet) => [...(fonts.get(stylesheet) ?? [])]))
        ...
        await writeFile(pageDir, contents.replace(/<head>.*<\/head>/s, `<head>${head}</head>`))
      }
    },
  },
}
